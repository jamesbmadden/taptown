...
}`;let he,ee;function Rt(e){he=e.getExtension("WEBGL_depth_texture"),console.log("[WEBGL Depth Texture Extension]",he);const i=e.createShader(e.VERTEX_SHADER);e.shaderSource(i,Et),e.compileShader(i);const o=e.createShader(e.FRAGMENT_SHADER);if(e.shaderSource(o,wt),e.compileShader(o),!e.getShaderParameter(i,e.COMPILE_STATUS))return alert("An error occurred compiling the vert shaders: "+e.getShaderInfoLog(i)),e.deleteShader(i),null;if(!e.getShaderParameter(o,e.COMPILE_STATUS))return alert("An error occurred compiling the frag shaders: "+e.getShaderInfoLog(o)),e.deleteShader(o),null;const r=e.createProgram();if(e.attachShader(r,i),e.attachShader(r,o),e.linkProgram(r),!e.getProgramParameter(r,e.LINK_STATUS))return alert("Unable to initialize the shader program: "+e.getProgramInfoLog(r)),null;ee={program:r,attribLocations:{vertexPosition:e.getAttribLocation(r,"aVertexPosition"),textureCoordPosition:e.getAttribLocation(r,"aTextureCoord"),normalPosition:e.getAttribLocation(r,"aNormal")},uniformLocations:{projectionMatrix:e.getUniformLocation(r,"uProjectionMatrix"),modelViewMatrix:e.getUniformLocation(r,"uModelViewMatrix"),normalMatrix:e.getUniformLocation(r,"uNormalMatrix"),texture:e.getUniformLocation(r,"uTexture"),highlight:e.getUniformLocation(r,"uHighlight")}}}function vt(e,i,o){Rt(e);const r=e.createFramebuffer(),n=e.createTexture();e.bindTexture(e.TEXTURE_2D,n),e.texImage2D(e.TEXTURE_2D,0,e.RGBA,i,o,0,e.RGBA,e.UNSIGNED_BYTE,null),e.texParameteri(e.TEXTURE_2D,e.TEXTURE_MIN_FILTER,e.LINEAR),e.texParameteri(e.TEXTURE_2D,e.TEXTURE_MAG_FILTER,e.LINEAR),e.texParameteri(e.TEXTURE_2D,e.TEXTURE_WRAP_S,e.CLAMP_TO_EDGE),e.texParameteri(e.TEXTURE_2D,e.TEXTURE_WRAP_T,e.CLAMP_TO_EDGE);const t=e.createTexture();e.bindTexture(e.TEXTURE_2D,t),e.texImage2D(e.TEXTURE_2D,0,e.DEPTH_COMPONENT,i,o,0,e.DEPTH_COMPONENT,e.UNSIGNED_INT,null),e.texParameteri(e.TEXTURE_2D,e.TEXTURE_MIN_FILTER,e.LINEAR),e.texParameteri(e.TEXTURE_2D,e.TEXTURE_MAG_FILTER,e.LINEAR),e.texParameteri(e.TEXTURE_2D,e.TEXTURE_WRAP_S,e.CLAMP_TO_EDGE),e.texParameteri(e.TEXTURE_2D,e.TEXTURE_WRAP_T,e.CLAMP_TO_EDGE),e.bindFramebuffer(e.FRAMEBUFFER,r),e.framebufferTexture2D(e.FRAMEBUFFER,e.COLOR_ATTACHMENT0,e.TEXTURE_2D,n,0),e.framebufferTexture2D(e.FRAMEBUFFER,e.DEPTH_ATTACHMENT,e.TEXTURE_2D,t,0);const d=e.checkFramebufferStatus(e.FRAMEBUFFER);return d!=e.FRAMEBUFFER_COMPLETE&&alert("The frame buffer failed: "+d.toString()),e.bindTexture(e.TEXTURE_2D,null),e.bindFramebuffer(e.FRAMEBUFFER,null),r}function Ft(e,i,{camera:o,buildingsPerRow:r,buildingsPerColumn:n,map:t,mapSize:d}){e.clearColor(0,0,0,1),e.clearDepth(1),e.enable(e.DEPTH_TEST),e.depthFunc(e.LEQUAL),e.clear(e.COLOR_BUFFER_BIT|e.DEPTH_BUFFER_BIT);const s=e.canvas.clientHeight/e.canvas.clientWidth,u=.1,c=100,f=R();se(f,0,r,0,r*s,u,c);const T=Math.floor(o.x/2)-1,x=Math.floor(o.z/2)-5,p=R();De(p,[o.x-10,2,o.z-13],[o.x,0,o.z],[0,1,0]);for(let m=x;m<x+n*2;m++){let Y=m*d;for(let b=T;b<T+r*2;b++){let X=Y+b;!(b>=d)&&!(b<0)&&!(m<0)&&!(m>=d)?W(e,t[X],[b,m],ee,f,p):W(e,254,[b,m],ee,f,p)}}}const F=document.getElementsByTagName("canvas")[0];let B=2*(innerWidth/200),C=2*(innerHeight/200),J,A,v=new dt,P=[-100,-100];window.camera=v;F.width=innerWidth*devicePixelRatio;F.height=innerHeight*devicePixelRatio;let te=innerWidth/B,Mt=innerHeight/C;v.setPixelToTileRatio(te,Mt);let a=F.getContext("webgl");window.addEventListener("resize",()=>{F.width=innerWidth*devicePixelRatio,F.height=innerHeight*devicePixelRatio,a.viewport(0,0,F.width,F.height),B=2*(innerWidth/200),C=2*(innerHeight/200);let e=innerWidth/B,i=innerHeight/C;v.setPixelToTileRatio(e,i)});F.addEventListener("mousemove",e=>{const i=e.clientX/te/2,o=(e.clientY-window.innerHeight)/te/2*1.425,r=[i,o];ae(r,r,[0,0],45*Math.PI/180);const n=[v.x/2,v.z/2+1];We(r,r,n),Ne(r,r),P=r});F.addEventListener("click",e=>{bt(P[0],P[1],e)});Ut();async function Ut(){await mt(),await y.setCallback(ce(c=>{J=c,A=Math.sqrt(J.length)})),await N.setCallback(ce(c=>{pt(c)}));const i=new URLSearchParams(location.search).get("save");i||(alert("Save not provided."),location.replace("../"));const o=await y.loadSave(i);o||(alert("Save couldn't be loaded."),location.replace("../")),document.title=`\u{1F3E0} ${i} \u2022 TapTown!`,console.log(o);const r=a.createShader(a.VERTEX_SHADER);a.shaderSource(r,je),a.compileShader(r);const n=a.createShader(a.FRAGMENT_SHADER);if(a.shaderSource(n,Qe),a.compileShader(n),!a.getShaderParameter(r,a.COMPILE_STATUS))return alert("An error occurred compiling the vert shaders: "+a.getShaderInfoLog(r)),a.deleteShader(r),null;if(!a.getShaderParameter(n,a.COMPILE_STATUS))return alert("An error occurred compiling the frag shaders: "+a.getShaderInfoLog(n)),a.deleteShader(n),null;const t=a.createProgram();if(a.attachShader(t,r),a.attachShader(t,n),a.linkProgram(t),!a.getProgramParameter(t,a.LINK_STATUS))return alert("Unable to initialize the shader program: "+a.getProgramInfoLog(t)),null;const d=vt(a,innerWidth,innerHeight),s={program:t,frameBuffer:d,attribLocations:{vertexPosition:a.getAttribLocation(t,"aVertexPosition"),textureCoordPosition:a.getAttribLocation(t,"aTextureCoord"),normalPosition:a.getAttribLocation(t,"aNormal")},uniformLocations:{projectionMatrix:a.getUniformLocation(t,"uProjectionMatrix"),modelViewMatrix:a.getUniformLocation(t,"uModelViewMatrix"),normalMatrix:a.getUniformLocation(t,"uNormalMatrix"),texture:a.getUniformLocation(t,"uTexture"),highlight:a.getUniformLocation(t,"uHighlight")}};await ct(a);const u=c=>{Ft(a,d,{camera:v,buildingsPerRow:B,buildingsPerColumn:C,map:J,mapSize:A}),It(s),requestAnimationFrame(u)};requestAnimationFrame(u)}function It(e){a.clearColor(0,0,0,1),a.clearDepth(1),a.enable(a.DEPTH_TEST),a.depthFunc(a.LEQUAL),a.clear(a.COLOR_BUFFER_BIT|a.DEPTH_BUFFER_BIT);const i=a.canvas.clientHeight/a.canvas.clientWidth,o=.1,r=100,n=R();se(n,0,B,0,B*i,o,r);const t=Math.floor(v.x/2)-1,d=Math.floor(v.z/2)-5;for(let s=d;s<d+C*2;s++){let u=s*A;for(let c=t;c<t+B*2;c++){let f=u+c;c===P[0]&&s===P[1]?a.uniform1i(e.uniformLocations.highlight,1):a.uniform1i(e.uniformLocations.highlight,0),!(c>=A)&&!(c<0)&&!(s<0)&&!(s>=A)?W(a,J[f],[c,s],e,n,v.cameraMatrix):W(a,254,[c,s],e,n,v.cameraMatrix)}}}
